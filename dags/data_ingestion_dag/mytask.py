...
def Spark_Process():
    mostBooked_room = pd.read_csv(f"{dag_path}/Spark/TopRoom_type.csv", low_memory=False)
    mostBooked_room.drop(columns=['GBP', 'EUR'])
    # save to csv
    mostBooked_room.to_csv(f"{dag_path}/Spark/mostBooked_room.csv", index=False)
...
def transform_data():
        ...
        mostBooked_room = pd.read_csv(f"{dag_path}/Spark/mostBooked_room.csv", low_memory=False)
        ...
        df = pd.concat([data, mostBooked_room], axis=1)

        # load processed data
        df.to_csv(f"{dag_path}/processed_data/processed_data.csv", index=False)
...
def load_data():
    ...
    c.execute('''
                CREATE TABLE IF NOT EXISTS booking_record (
                    ...
                    hotel_name TEXT(512),
                    mostBooked_room TEXT(512)
                );
             ''')
    records = pd.read_csv(f"{dag_path}/processed_data/processed_data.csv")
    records.to_sql('booking_record', conn, index=False, if_exists='replace')
...
end_session = DummyOperator(
    task_id = 'end_session',
    dag = ingestion_dag
)

start_session >> task_1 >> task_2
task_2 >> [task_3, end_session]
